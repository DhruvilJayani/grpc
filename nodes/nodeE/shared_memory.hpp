...
private:
    ...
    void initialize() {
        ...
        std::cout << "NodeE: Using shared memory file: " << abs_path << std::endl;
        
        // First check if file exists
        struct stat st;
        bool file_exists = (stat(abs_path.c_str(), &st) == 0);
        std::cout << "NodeE: File exists: " << (file_exists ? "yes" : "no") << std::endl;
        
        // Create or open the file
        fd_ = open(abs_path.c_str(), O_CREAT | O_RDWR, 0666);
        if (fd_ == -1) {
            std::cerr << "NodeE: Failed to open file: " << abs_path << " (errno: " << errno << ")" << std::endl;
        ...
        std::cout << "NodeE: Shared memory initialized. Current counter: " << data_->counter << std::endl;
    }
